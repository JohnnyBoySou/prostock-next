...
export default function DashboardPage() {
  ...
  const { data: line, isLoading: loadingDay, refetch } = useQuery({
    queryKey: ["stores report graph", id, fornecedor, tab, dateC, dateF],
    queryFn: async () => {
  ...
  const { data: graph, isLoading: loadinExcel, refetch: handleExcel } = useQuery({
    queryKey: ["stores report graph", id, fornecedor, tab, dateC, dateF],
    queryFn: async () => {
      const res = await showReportExcel(produto, id, fornecedor, dateC, dateF, tab);
      return res;
    },
    cacheTime: 0,
    enabled: false
  ...
  return (
    <div className="space-y-6">
      <Store item={store} token={line?.token} handleExcel={handleExcel} types={types} tab={tab} settab={settab} setdateC={setdateC} setdateF={setdateF} handleSearch={handleSearch}   dateC={dateC} dateF={dateF}
...
const Store = ({ item, fornecedor, tab, settab,  types, setfornecedor, setproduto, produto, setdateC, setdateF, dateC, dateF, handleSearch, handleExcel }) => {
  if (!item) return null;
